...
export function TaskDetailModal({ taskId, onClose }: TaskDetailModalProps) {
  ...
  const { data: updatesData } = useQuery(GET_TASK_UPDATES, {
    variables: { taskId: taskId! },
    skip: !taskId
  });

  const [addTaskUpdate] = useMutation(ADD_TASK_UPDATE, {
    refetchQueries: ['GetTaskUpdates', 'GetTaskDetails']
  });

  const [updateTask] = useMutation(UPDATE_TASK_WITH_FILES, {
    refetchQueries: ['GetTaskDetails', 'GetMyTasks', 'GetTeamTasks']
  });
  ...
  const taskUpdates = updatesData?.taskUpdates;

  const isCompleted = task?.completed || false;
  
  // Logic permissions: can complete if assigned or created
  const isAssigned = task?.assignedMembers?.some((m: any) => m.id === user?.id);
  // Assuming createdBy is populated or will be. 
  // Need to add createdBy to query if not present for permission check?
  // Our query fetches assignedMembers. Let's check schema.
  
  const canComplete =
    task &&
    !isCompleted &&
    (isAssigned); // Should also check creator. Add createdBy to query if strictly needed.
              ...
              {isCompleted &&
                ...
                task.completionFiles.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {task.completionFiles.map((file: any, idx: number) => (
                        <div
                          key={idx}
