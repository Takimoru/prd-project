...
export function LogsheetReviewPage() {
  ...
  return (
    <div className="space-y-8">
      <AdminHeader
        title="Tinjauan Logbook"
        description="Pantau dan tinjau rekap kemajuan mingguan yang dikirimkan oleh tim."
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent>
            ...
            >
              <SelectTrigger className="w-full bg-background transition-all">
                <SelectValue placeholder={loadingPrograms ? "Memuat periode..." : "Pilih periode"} />
              </SelectTrigger>
              <SelectContent>
                {programs.map((p: any) => (
        ...
        <Card className={cn(
          ...
          <CardContent>
            ...
            >
              <SelectTrigger className="w-full bg-background transition-all">
                <SelectValue placeholder={!selectedProgram ? "Pilih periode terlebih dahulu" : loadingTeams ? "Memuat tim..." : "Pilih tim"} />
              </SelectTrigger>
              <SelectContent>
                {teams.map((t: any) => (
        ...
        {!selectedTeam ? (
          <div className="text-center py-24 bg-muted/5 rounded-xl border border-dashed border-muted-foreground/20 m-6">
            ...
            <p className="text-muted-foreground max-w-xs mx-auto mt-2">
              Silakan pilih periode dan kemudian pilih tim untuk meninjau pengajuan logbook mingguan mereka.
            </p>
          </div>
        ) : loadingLogsheets ? (
            ...
            <div className="p-6 border-b bg-muted/30 flex justify-between items-center">
              <h2 className="text-xl font-bold flex items-center gap-2 text-foreground">
                ...
                <span className="text-xs font-normal text-muted-foreground bg-background px-2 py-0.5 rounded-full border">
                  {filteredLogs.length} ditemukan
                </span>
              </h2>
              <div className="relative w-64 text-sm">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input 
                  placeholder="Cari berdasarkan minggu atau nama..." 
                  className="pl-9 h-9"
                  value={searchTerm}
            ...
            <div className="p-0">
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow>
                    <TableHead className="w-[120px]">Minggu</TableHead>
                      ...
                      <TableRow key={log.id} className="group hover:bg-primary/5 transition-colors">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-sm">{log.createdBy?.name || "Anonim"}</span>
                          </div>
                        </TableCell>
