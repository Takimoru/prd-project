    ...
    >
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Dikirim: {format(new Date(registration.submittedAt), "dd MMM yyyy")}
        </p>
        
        {isApproved && registration.reviewedAt && (
          <p className="text-xs text-gray-500">
            Disetujui: {format(new Date(registration.reviewedAt), "dd MMM yyyy")}
            {registration.reviewedBy && (
              <span> oleh {registration.reviewedBy.name || "Admin"}</span>
            )}
          </p>
        )}
        ...
        {isApproved && registration.user && (
          <p className="text-xs text-green-600 mt-1">
            ✓ Akun pengguna aktif: {registration.user.name}
          </p>
        )}
      </div>
